...
def make_intra_z_merge(BB_code, logical_indices=(0, 1), shuttling_threshold=2, plot=False, **merge_kwargs):
...
def prepare_z_logical_state(BB_code, merged):
    """Prepare one BB code block in |0_L>^k and all edge/adapter qubits in |+>."""
    ...
    for logical in BB_code.get_logical_ops(Pauli.Z):
        ...
        stabilizers.append(pauli_string_on_qubits("Z", np.flatnonzero(logical), n_total))
    ...
    plus_qubits = edge_and_adapter_qubits(merged)
...
def normal_bb_checks_and_ancillas(BB_code, merged):
    ...
    n_x = Hx.shape[0]
    x_ancillas = {row: int(merged["x_check_to_ancilla"][row]) for row in range(n_x)}
...
def joint_z_observable_rows(merged):
    """Rows in merged z_checks whose parity is the multi-logical Z product."""
    rows = []
...
def intra_z_merge_measurement_circuit(
    BB_code,
    logical_indices=(0, 1),
    bb_rounds=4,
    ...
    **merge_kwargs,
    ...
    """Measure the product of several Z logicals in one BB code block."""
    merged = make_intra_z_merge(
        BB_code,
    ...
    circuit = prepare_z_logical_state(BB_code, merged)
...
def estimate_intra_z_merge_error_rate(
    BB_code,
    p,
    logical_indices=(0, 1),
    bb_rounds=4,
    ...
    **merge_kwargs,
    ...
    circuit, merged = intra_z_merge_measurement_circuit(
        BB_code,
        logical_indices=logical_indices,
        bb_rounds=bb_rounds,
...
def plot_intra_z_merge_sweep(result, ax=None):
    import matplotlib.pyplot as plt

    ps = result["ps"]
    raw = result["raw"]
    decoded = result["decoded"]
    label = "".join(f"Z{int(i)}" for i in result["logical_indices"])
