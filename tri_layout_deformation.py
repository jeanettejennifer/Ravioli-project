"""Triangular-lattice layout and stabilizer lift for auxiliary surgery.

Usage:
----------------
    tri_layout(def_res)

``def_res`` is the dictionary returned by ``deform_code_for_logical`` in
``graph_helper_functions.py``.  

The pipeline is:

1. Planarize the abstract auxiliary graph.
   Crossing abstract edges are split by adding vertices. The
   stabilizer matrices are rebuilt for the split graph.

2. Place graph vertices on a triangular lattice.
   Make Tutte-style embedding of graph, fixing the largest cycle as an outer boundary,
   solves barycentric positions for the remaining vertices, then snaps those
   positions to distinct triangular-lattice sites.  

3. Convert graph edges from def_res as physical lattice paths.
   Each abstract edge becomes a shortest path of lattice edges. Each lattice
   edge is one auxiliary qubit.

4. Convert stabilizers to the lattice.
   Vertex checks, cycle checks, and deformed old opposite-basis checks are
   rewritten so their auxiliary support is expressed in the triangular lattice-edge qubits.
...
# -----------------------------------------------------------------------------
# Small geometry and GF(2) helpers
# -----------------------------------------------------------------------------
...
def _crossing_edges(G, pos):
    ...
    for i, e1 in enumerate(edges):
        for e2 in edges[i + 1:]:
            if set(e1) & set(e2):
                continue
            if _segments_cross(pos[e1[0]], pos[e1[1]], pos[e2[0]], pos[e2[1]]):
...
def _lift_old_graph_matrix(deformation_result, g_planar, M_old):
...
def _rebuild_deformation_checks(deformation_result, g_planar):
    ...
    H_basis_old = _lift_old_graph_matrix(deformation_result, g_planar, deformation_result["H_basis_old"])
    H_opposite_basis_old_padded = _lift_old_graph_matrix(
        deformation_result,
        g_planar,
...
def planarize_deformation(deformation_result, layout_seed=7):
    ...
    while True:
        ...
        crossings = _crossing_edges(G_current, pos)
        if not crossings:
            break
    ...
    H_basis_new, H_opposite_basis_new, H_basis_old, H_opposite_basis_old_padded = _rebuild_deformation_checks(
        deformation_result,
        g_planar,
    )
...
def _triangular_lattice(rows, cols, spacing=1.0):
    L = nx.Graph()
    pos = {}
    for r in range(rows):
        for c in range(cols):
...
def _largest_cycle_nodes(G):
    cycles = nx.cycle_basis(G)
    return max(cycles, key=len) if cycles else None


def _tutte_positions(G, fixed_cycle=None):
...
def _scale_to_lattice(tutte_pos, lattice_pos, margin=1):
...
def _nearest_free_sites(target_pos, lattice_pos, rng=None, jitter=0.0):
...
def _placement_energy(G, assignment, lattice_graph, lattice_pos, target_pos, anchor_weight=0.03, bbox_weight=0.35):
...
def _improve_vertex_sites(G, assignment, lattice_graph, lattice_pos, target_pos, rng=None, rounds=5):
    """Greedily move vertices to nearby free sites if the placement score drops."""
    rng = np.random.default_rng(0) if rng is None else rng
    assignment = dict(assignment)
    best = _placement_energy(G, assignment, lattice_graph, lattice_pos, target_pos)
        ...
        for u in nodes:
            ...
            for site in candidates[:16]:
                if site == old:
                    continue
                assignment[u] = site
                score = _placement_energy(G, assignment, lattice_graph, lattice_pos, target_pos)
                if score + 1e-9 < best:
                    used.remove(old)
                    used.add(site)
                    old = site
...
def _route_abstract_edges(G, lattice_graph, vertex_sites, rng=None, route_randomness=0.03):
...
def _graph_edge_order(deformation_result):
...
def _all_pairings(items):
    ...
    for i in range(1, len(items)):
        ...
        for tail in _all_pairings(rest):
            yield [(first, second)] + tail


def _bfs_shortest_path(lattice_graph, a, b, participation=None, max_participation=None):
...
def _path_congestion_score(path, participation):
...
def _choose_old_bb_support(lattice_graph, terminals, participation, max_participation=7):
    ...
    best = None
    for pairs in _all_pairings(terminals):
        paths = []
        for a, b in pairs:
            paths.append(_bfs_shortest_path(
                lattice_graph,
                a,
                b,
        ...
        support = _xor_path_edges(paths)
        path_lengths = [len(path) - 1 for path in paths]
        congestion = [_path_congestion_score(path, participation) for path in paths]
...
def _ordered_cycle(edge_support):
...
def _decompose_even_support(edge_support):
    G = nx.Graph()
...
def _split_large_cycle_supports(cycle_supports, lattice_graph, max_weight=6):
    rows = []
    split_count = 0
    for support, meta in cycle_supports:
        comps = _decompose_even_support(support)
        if len(comps) > 1:
    ...
    while rows:
        ...
        for i in range(k):
            for j in range(i + 2, k):
                ...
                chord = set(_path_edges(chord_path))
...
def _build_lattice_stabilizers(
    deformation_result,
    lattice_graph,
    vertex_sites,
    abstract_routes,
    ...
    """
    n_data = int(deformation_result["n_original_qubits"])
    graph_edges = _graph_edge_order(deformation_result)
    ...
    for row in M_old_opp:
        ...
        support, paths = _choose_old_bb_support(
            lattice_graph,
            terminals,
            participation,
    ...
    cycle_supports, split_count = _split_large_cycle_supports(raw_cycles, lattice_graph, max_weight=max_cycle_weight)
...
def _distribution(values):
    ...
    return dict(sorted((int(x), int(np.sum(values == x))) for x in np.unique(values)))


def _weight_summary(M):
    w = np.sum(np.asarray(M, dtype=np.uint8), axis=1).astype(int)
    return {
        "weights": w.tolist(),
        "min": int(w.min()) if len(w) else 0,
        "max": int(w.max()) if len(w) else 0,
        "distribution": _distribution(w),
    }


def _qubit_participation_summary(*matrices):
    H = np.vstack([np.asarray(M, dtype=np.uint8) for M in matrices if np.asarray(M).size])
    c = np.sum(H, axis=0).astype(int)
    return {
        "counts": c.tolist(),
        "min": int(c.min()) if len(c) else 0,
        "max": int(c.max()) if len(c) else 0,
        "distribution": _distribution(c),
    }


def print_lattice_stabilizer_summary(stabilizers, high_weight_threshold=None):
    """Print stabilizer weights and qubit participation statistics."""
    basis = _weight_summary(stabilizers["H_basis_def"])
    opp = _weight_summary(stabilizers["H_opposite_basis_def"])
    part = _qubit_participation_summary(stabilizers["H_basis_def"], stabilizers["H_opposite_basis_def"])
    print("lattice edge qubits:", stabilizers["n_lattice_edge_qubits"])
    print("total qubits:", stabilizers["n_total_qubits"])
    print("basis stabilizer weight distribution:", basis["distribution"], "max:", basis["max"])
    print("opposite stabilizer weights:", opp["weights"])
    print("opposite stabilizer weight distribution:", opp["distribution"], "max:", opp["max"])
    if high_weight_threshold is None:
        high_weight_threshold = opp["max"]
    print(f"opposite checks with weight >= {high_weight_threshold}:")
    for r, w in enumerate(opp["weights"]):
        if w >= high_weight_threshold:
            src = stabilizers["opposite_row_sources"][r]
            print(f"  row {r}: weight {w}, {src['type']} local_row={src['local_row']}")
    print("qubit check participation distribution:", part["distribution"], "max:", part["max"])
    print("cycle splits:", stabilizers.get("cycle_split_count", 0))
    return {"basis": basis, "opposite": opp, "participation": part}


def plot_lattice_opposite_check_route(tutte_state, row=None, ax=None):
    """Visualize one opposite-basis check on the triangular lattice."""
    stabilizers = tutte_state["lattice_stabilizers"]
    H = stabilizers["H_opposite_basis_def"]
    weights = np.sum(H, axis=1).astype(int)
    if row is None:
        row = int(np.argmax(weights))
    lattice_pos = tutte_state["lattice_pos"]
    materialized_graph = stabilizers["materialized_graph"]
    vertex_sites = stabilizers["original_vertex_to_site"]
    n_data = stabilizers["n_data_qubits"]
    source = stabilizers["opposite_row_sources"][row]
    support = np.where(H[row] == 1)[0]
    data_support = [int(q) for q in support if q < n_data]
    lattice_support = [int(q) for q in support if q >= n_data]
    q_to_edge = {q: e for e, q in stabilizers["lattice_edge_to_qubit"].items()}
    if ax is None:
        fig, ax = plt.subplots(figsize=(8, 8))
    else:
        fig = ax.figure
    for a, b in tutte_state["lattice_graph"].edges():
        pa, pb = lattice_pos[a], lattice_pos[b]
        ax.plot([pa[0], pb[0]], [pa[1], pb[1]], color="0.92", linewidth=0.6, zorder=0)
    for a, b in materialized_graph.edges():
        pa, pb = lattice_pos[a], lattice_pos[b]
        ax.plot([pa[0], pb[0]], [pa[1], pb[1]], color="#9ecae1", linewidth=1.0, alpha=0.55, zorder=1)
    for q in lattice_support:
        a, b = q_to_edge[q]
        pa, pb = lattice_pos[a], lattice_pos[b]
        ax.plot([pa[0], pb[0]], [pa[1], pb[1]], color="#d62728", linewidth=4.0, solid_capstyle="round", zorder=3)
        mid = (pa + pb) / 2
        ax.text(mid[0], mid[1], str(q), color="#7f0000", fontsize=7, ha="center", va="center", zorder=4)
    pts = np.array([lattice_pos[vertex_sites[v]] for v in tutte_state["G"].nodes()])
    ax.scatter(pts[:, 0], pts[:, 1], s=85, color="#f28e2b", edgecolor="black", linewidth=0.8, zorder=5)
    for v in tutte_state["G"].nodes():
        x, y = lattice_pos[vertex_sites[v]]
        ax.text(x, y + 0.18, str(v), ha="center", va="bottom", fontsize=8, zorder=6)
    ax.set_aspect("equal")
    ax.axis("off")
    ax.set_title(f"Opposite check row {row}: weight {int(weights[row])}, {source['type']}\ndata support={data_support}, lattice-edge support={lattice_support}")
    fig.tight_layout()
    return fig, ax, {"row": row, "weight": int(weights[row]), "source": source, "data_support": data_support, "lattice_edge_support": lattice_support}


# -----------------------------------------------------------------------------
# Stage 2 public helper: embed and draw the lattice layout
# -----------------------------------------------------------------------------


def _candidate_grid_sizes(n, rows, cols, grid_extra=4):
...
def embed_tutte_style(
    ...
    """Place an auxiliary graph on a triangular lattice and draw it.

    This performs the geometric part of the pipeline: continuous Tutte layout,
    snapping to lattice sites, local compaction, abstract-edge routing, optional
    stabilizer lift, and plotting.
    """
    G = _as_networkx_graph(g)
    ...
    if deformation_result is not None:
        ...
        if fixed_cycle is None and "original_g" in deformation_result:
            fixed_cycle = _largest_cycle_nodes(_as_networkx_graph(deformation_result["original_g"]))
    tutte_pos, tutte_fixed_cycle = _tutte_positions(G, fixed_cycle=fixed_cycle)
    root_rng = np.random.default_rng(rng_seed)
    best = None
    for rr, cc in _candidate_grid_sizes(n, rows, cols):
        lattice_graph, lattice_pos = _triangular_lattice(rr, cc, spacing=spacing)
        target_pos = _scale_to_lattice(tutte_pos, lattice_pos, margin=1 if min(rr, cc) <= 6 else 2)
        for restart in range(max(1, random_restarts)):
            rng = np.random.default_rng(int(root_rng.integers(0, 2**32 - 1)))
            init = _nearest_free_sites(target_pos, lattice_pos, rng=rng, jitter=placement_jitter if restart else 0.0)
            vertex_sites, placement_score = _improve_vertex_sites(G, init, lattice_graph, lattice_pos, target_pos, rng=rng)
            routes = _route_abstract_edges(G, lattice_graph, vertex_sites, rng=rng, route_randomness=route_randomness if restart else 0.0)
            stats = _route_stats(routes)
            score = 10 * rr * cc + stats["total_route_length"] + 2 * stats["max_route_length"] + placement_score
            cand = (score, rr * cc, stats["total_route_length"], stats["max_route_length"], rr, cc, restart, lattice_graph, lattice_pos, target_pos, vertex_sites, routes, placement_score, stats)
            if best is None or cand[:6] < best[:6]:
                best = cand
    _, _, _, _, rows, cols, best_restart, lattice_graph, lattice_pos, target_pos, vertex_sites, routes, placement_score, stats = best
    ...
    if deformation_result is not None:
        state["lattice_stabilizers"] = _build_lattice_stabilizers(
            deformation_result,
            lattice_graph,
            vertex_sites,
            routes,
    ...
    for a, b in lattice_graph.edges():
        pa, pb = lattice_pos[a], lattice_pos[b]
        ax.plot([pa[0], pb[0]], [pa[1], pb[1]], color="0.90", linewidth=0.6, zorder=0)
    draw_edges = state.get("materialized_edges", [e for path in routes.values() for e in _path_edges(path)])
    for a, b in draw_edges:
        pa, pb = lattice_pos[a], lattice_pos[b]
        ax.plot([pa[0], pb[0]], [pa[1], pb[1]], color="#2f6f9f", linewidth=2.0, alpha=0.85, zorder=2)
    pts = np.array([lattice_pos[vertex_sites[v]] for v in G.nodes()])
    ax.scatter(pts[:, 0], pts[:, 1], s=90, color="#f28e2b", edgecolor="black", linewidth=0.8, zorder=3)
    for v in G.nodes():
        x, y = lattice_pos[vertex_sites[v]]
        ax.text(x, y + 0.18, str(v), ha="center", va="bottom", fontsize=8, zorder=4)
    ax.set_aspect("equal")
    ax.axis("off")
    ax.set_title("Physical lattice deformation graph after Tutte placement and rerouting")
    fig.tight_layout()
    return fig, ax, state
...
def tri_layout(
    ...
    """Run the full triangular-lattice deformation layout pipeline.

    Parameters
    ----------
    deformation_result:
        Dictionary returned by ``deform_code_for_logical``.

    Returns
    -------
    dict
        Notebook-friendly result bundle.  The most useful keys are:

        ``fig``, ``ax``
            Matplotlib visualization of the materialized lattice graph.
        ``H_basis``, ``H_opposite_basis``
            Full updated stabilizer matrices after lattice conversion.
        ``H_basis_old``, ``H_basis_new``
            Old basis checks and new vertex checks.
        ``H_opposite_basis_old_padded``, ``H_opposite_basis_new``
            Old opposite-basis checks with routed support and new cycle checks.
        ``logical_observable_rows``
            Rows of ``H_basis`` whose product equals the target logical.
        ``port_vertex_rows_global``
            Global row indices of vertex checks attached to logical-support
            qubits.
    """
    planarized = planarize_deformation(deformation_result)
    fig, ax, state = embed_tutte_style(
        planarized["g"],
    ...
    logical_observable_row_coeffs = _gf2_row_combination_for_target(
        stabilizers["H_basis_def"],
        logical_full,
    )
    logical_observable_rows = np.where(logical_observable_row_coeffs == 1)[0].astype(int).tolist()
