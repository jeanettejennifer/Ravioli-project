...
def deform_old_x_checks_with_graph_edges(
    ...
    # Keep only actual X-check rows
    Hx_old = Hx_all[np.any(Hx_all, axis=1)]
    Hx_old_padded = np.pad(Hx_old, ((0, 0), (0, n_edges)), mode="constant")

    # Map BB qubit index on logical support -> logical vertex index 0..n_L-1
    ...
    for full_row in range(Hx_all.shape[0]):
        ...
        # Which BB qubits in this X check lie on the chosen logical support?
        overlap = np.where((logical_qubits == 1) & (row == 1))[0]

        # In your construction, only checks with exactly 2 overlaps define edges
        if len(overlap) == 2:
            q1, q2 = map(int, overlap)
            l1 = bb_to_logical_vertex[q1]
            l2 = bb_to_logical_vertex[q2]

            # The graph edge is between check-side vertices C_l1 and C_l2
            u = n_L + l1
            v = n_L + l2
            ...
            # Add X support on the ancilla qubit for this graph edge
...
def find_short_cycle_basis(pairs, return_edges=True):
    ...
    # ------------------------------------------------------------
    # 1. Build simple undirected graph
    # ------------------------------------------------------------
    adj = defaultdict(set)
    edges = set()
    ...
    def canonical_cycle_vertices(path):
        ...
        """
        cyc = path[:]  # already [u,...,v]
        # normalize cyclic representation for duplicate removal
    ...
    # Incremental GF(2) independence test using bitset elimination
    basis = {}  # pivot_bit -> vector_bitset
    ...
    # ------------------------------------------------------------
    # 2. Generate shortest-cycle candidates from each edge
    # ------------------------------------------------------------
    candidates = []
    seen_cycles = set()
    ...
    for e in edge_list:
        ...
        # Need at least 2 edges in the path, otherwise would be a multiedge/2-cycle
        if len(path) < 3:
            continue
    ...
    # ------------------------------------------------------------
    # 3. Sort shortest first, then greedily add independent cycles
    # ------------------------------------------------------------
...
def gf2_solve_left(H, v):
    ...
    rows, cols = A.shape  # rows=n, cols=m+1
    pivot_cols = []
    r = 0
    for c in range(m):
        pivot = None
